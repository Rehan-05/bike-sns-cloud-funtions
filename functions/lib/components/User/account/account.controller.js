...
const updateAccount = async (req, res, next) => {
    try {
        ...
        if (password && email && newPassword) {
            ...
            // also update updated at field in admin collection 
            v1_1.logger.info(`User updated with uid: ${result}`);
            const admin = await config_1.db.collection('Admin').doc(result.uid).update({ updated_at: dayjs().format('YYYY-MM-DD HH:mm:ss'), password: newPassword });
            //  log now update the admin collection
