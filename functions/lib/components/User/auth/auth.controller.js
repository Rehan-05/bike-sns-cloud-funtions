...
const login = async (req, res, next) => {
    try {
        ...
        const person = await config_1.db.collection('User').doc(user.user.uid).get();
        if (!person.exists) {
            return res.status(404).json((0, ApiErrorStructure_1.ERROR)(auth_message_1.authMessages.notFound, 404, 1000));
        }
...
const registrationUser = async (req, res, next) => {
    ...
    try {
        ...
        if (err instanceof apiErrorHandler_1.HttpException)
            ...
            throw (0, apiErrorHandler_1.badImplementationException)(err);
    }
};
