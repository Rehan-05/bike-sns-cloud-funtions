...
const login = async (req, res, next) => {
    try {
        ...
        const agency = await config_1.db.collection('Agency').doc(user.user.uid).get();
        if (!agency.exists) {
        ...
        res.status(200).json({ accessToken: token, refreshToken: refreshToken, email: user.user.email, uid: user.user.uid, type: 'agency' });
    }
    catch (err) {
...
const forgotPassword = async (req, res, next) => {
    try {
        ...
        const user = await config_1.db.collection('Agency').where('email', '==', email).get();
        if (user.empty) {
            return res.status(401).json((0, ApiErrorStructure_1.ERROR)('User not found', 401, 1000));
        }
        // if agency name is admin then return error
        if (user.docs[0].data().name === 'Admin') {
            return res.status(401).json((0, ApiErrorStructure_1.ERROR)('You are not allowed to reset password', 401, 1000));
        }
